...
# Here you import the data, dluble check it imported correctly and have a quick look at the variables.
# Be clear on the problem you are answering and the variables it is linked to.

### Import Data
sendy_raw <- as_tibble(read.csv("Masked_Train.csv"))

#Have a quick look at it from top to bottom and left to right to reassure the fact that the data was loaded. 
View(sendy_raw)
# OR
head(sendy_raw)

# Another handy way to view your data ... 
glimpse(sendy_raw) 
...
sendy_data <- sendy_raw %>% 
  mutate(Placement...Time = parse_time(as.character(Placement...Time), '%I:%M:%S %p'),
         ...
         Delivery.at.destination...Time = parse_time(as.character(Delivery.at.destination...Time), '%I:%M:%S %p'))
...
summary(sendy_data$Time.from.Pickup.to.Arrival)
...
sendy_data <- sendy_data %>% mutate(delivery_time_minutes = Time.from.Pickup.to.Arrival/60)
ggplot(data = sendy_data) +
...
deliveries_under_5 <- sendy_data %>% filter(delivery_time_minutes <= 5) %>% 
...
#let's look at all the deliveries that took under say 5 minutes
deliveries_under_10 <- sendy_data %>% filter(delivery_time_minutes <= 10) %>% 
  select(ends_with("Time"), Time.from.Pickup.to.Arrival, delivery_time_minutes) %>% arrange(delivery_time_minutes)
...
sendy_data <- sendy_data %>% filter(Time.from.Pickup.to.Arrival > 180)
...
sendy_data <- sendy_data %>% mutate(Temperature= replace_na(Temperature, mean(Temperature, na.rm=T)))
...
sendy_numeric <- select_if(sendy_data, is.numeric) # select numeric variables
...
names(sendy_data)
sendy_month_days <- sendy_data %>% select(ends_with("Day.of.Month"))
sendy_month_days
...
sendy_data <- sendy_data %>% select(-delivery_time_minutes)

# select numeric variables
sendy_numeric <- select_if(sendy_data, is.numeric)
...
sendy_processed <- sendy_data %>% select(Order.No, Order.Week.Day, Arrival.at.Pickup...Time, Distance..KM., 
                                         Temperature, Precipitation.in.millimeters, Time.from.Pickup.to.Arrival)
...
# TRANSFORMATIONS
#We arent going to do any trsansformations for now
...
part.index <- createDataPartition(sendy_processed$Time.from.Pickup.to.Arrival, p = 0.75, list = FALSE) # create random order to split the data
...
model_lm <- train(Time.from.Pickup.to.Arrival ~ ., data=sendy_train, method='lm')
predicted <- predict(model_lm, sendy_test[,-length(sendy_test)]) # predict without the response variable column (obviously because we are testing!)
...
# Hyperparameter selection
  # Caret was actually doing hyperparameter selection without us asking! (although we didnt need it for the lm)
...
#### 6) MODEL VALIDATION -----------------------------------------------------------------------------------
 ...
 pred_rf <- predict.train(model_rf, sendy_test[,-length(sendy_test)])
 ...
 pred_RMSE <- data.frame(LM = RMSE(pred_lm, sendy_test[,length(sendy_test)]),
                         ...
                         RF = RMSE(pred_rf, sendy_test[,length(sendy_test)]),
